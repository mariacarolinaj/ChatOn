...
public class MensagemDAOImpl implements MensagemDAO{
    ...
    public List<Mensagem> listAll() {
        try {
            ...
            String sql = "SELECT * FROM mensagem ORDER BY codMensagem;";
            ...
            if (rs.next()) {
                do {
                    Mensagem mensagem = new Mensagem(rs.getString("txtMensagem"), userDAO.getUsuarioById(rs.getInt("codUsuario")), 
                        salaDAO.getSalaById(rs.getInt("codSala")), rs.getInt("codMensagem"), rs.getDate("datMensagem"));
                } while (rs.next());
            }
